#!/usr/bin/env bash
#
# rqbit/mayhem/build.sh — build the bencode cargo-fuzz `roundtrip` harness as a sanitized libFuzzer
# binary, replicating OSS-Fuzz's Rust path (base-builder-rust's `compile` + a `cargo fuzz build`).
#
# rqbit's `librqbit-bencode` crate is a pure-Rust bencode (de)serializer. The roundtrip harness
# (mayhem/fuzz/fuzz_targets/roundtrip.rs) parses arbitrary bytes into a dynamic BencodeValue, then
# re-serializes + re-parses and asserts equality — a classic parse/serialize fixpoint check.
#
# Because upstream ships NO fuzz crate, the cargo-fuzz crate lives ADDITIVELY under mayhem/fuzz/
# (its own isolated [workspace]) and depends on the in-tree bencode crate by path. cargo-fuzz:
#   - ships its own libFuzzer runtime (the produced binary IS a libFuzzer target — `libfuzzer: true`);
#   - enables ASan the Rust way via RUSTFLAGS `-Zsanitizer=address` (NOT clang's $SANITIZER_FLAGS /
#     CFLAGS — those don't apply to rustc), which is what OSS-Fuzz's `compile` sets for Rust. nightly
#     is required for `-Zsanitizer`.
#
...
# resolves every crate from that cache. So we do NOT hard-code `--offline` (it would break this first
...
# sections once so it contributes no debug info; our project code (DWARF 2 via RUST_DEBUG_FLAGS) then
# appears first. The stripped .a is baked into the image, so the offline re-run reproduces it.
...
# libfuzzer-sys compiles libFuzzer from C++ via the cc crate; force DWARF 3 so those CUs also satisfy
# the check (the cc crate respects CFLAGS/CXXFLAGS). On the re-run these are the same, so cargo reuses
# the cached libfuzzer.a (fingerprint stable).
...
# combined with the stripped ASan runtime this keeps the first .debug_info CU < 4. (For parity we
# note the base's clang $SANITIZER_FLAGS is intentionally not used here — rustc ignores clang flags.)
...
# `-O` (release w/ opt) + `--debug-assertions` mirrors OSS-Fuzz's build.sh. Use the image's DEFAULT
# toolchain (pinned to the required nightly by the Dockerfile); a `+toolchain` override would make
# rustup try to install a different channel into the locked /opt/toolchains/rust.
...
ls -la /mayhem/roundtrip 2>&1 || true
